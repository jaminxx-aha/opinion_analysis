...
def generate_report(input_path: str, output_path: str = None) -> str:
    ...
    parts.append('<div class="chart-box"><h3>一级分类分布</h3><div class="chart-wrap"><canvas id="level1Chart"></canvas></div></div>')
    parts.append('<div class="chart-box"><h3 id="pie-title">二级分类分布</h3><div class="chart-wrap"><canvas id="pieChart"></canvas></div></div>')
    parts.append('</div>')
    parts.append('</div>')
    ...
    parts.append('data: { labels: Object.keys(stats.level1), datasets: [{ data: Object.values(stats.level1), backgroundColor: ["#3b82f6","#8b5cf6","#ec4899","#f43f5e","#06b6d4","#14b8a6","#16a34a","#ca8a04"] }] },')
    parts.append('options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, onClick: (e,els) => { if(els.length>0) setFilter("level1", charts.level1.data.labels[els[0].index]); } }')
    parts.append('});')

    parts.append('charts.pie = new Chart(document.getElementById("pieChart"), {')
    parts.append('type: "doughnut",')
    parts.append('data: { labels: Object.keys(stats.level2), datasets: [{ data: Object.values(stats.level2), backgroundColor: ["#3b82f6","#8b5cf6","#ec4899","#f43f5e","#06b6d4","#14b8a6","#16a34a","#ca8a04"] }] },')
